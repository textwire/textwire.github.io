...
> [!NOTE]
> This is not an upgrade guide, it's an article that goes over all the changes in version 4 and explains why they were made. If you are transitioning from Textwire v3, you can follow this [Upgrade Guide](/v4/upgrade) for all the instructions. v4 contains several breaking changes, so make sure you don't miss any parts of the guide.
...
This is true for many other programming languages like PHP, C, C++, C#, Java, and many more. It's not specific to JavaScript. Remember that JavaScript was built on the foundation of C, so it inherited a lot of its syntax and behavior from C. The postfix increment operator is one of those features that has been around for a long time and is widely used in many programming languages.
...
In other words, increment/decrement is actually a shortcut for:

1. Return variable `return i`
2. Reassign variable with the new value plus 1 `i = i + 1`
...
Unlike other programming languages, Go decided to go a different route. Instead of confusing developers with post-increment, Go treats increment and decrement as statements, not expressions. This means that you can't use them in places where an expression is expected, such as in a function call or as part of a larger expression. In Go's view, `n++` is the same as `n = n + 1`, and it doesn't return a value since it's a statement, not an expression.
...
`@continueIf` and `@breakIf` directives are now should be written lowercase as `@continueif` and `@breakif` to match consistency with other directives like `@slotif` and `@elseif`.
