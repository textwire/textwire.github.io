...
## Directives
Directives are special Textwire statements that begin with the `@` symbol. They are used to define layouts, insert content into reserved placeholders, and perform logical operations such as conditionals and loops. Directives can only be placed within HTML code and are not allowed inside braces statements (`&lparen;{` and `}}`).
...
## Braces Statements
Braces statements are special Textwire constructs that begin with `&lparen;{` and end with `}}`. They are used for defining variables, performing arithmetic operations, conditionally rendering content, and more. Braces statements can be placed anywhere in the file, except within directives.
...
- **Return Values**: All braces statements return a string.
  - Defining a variable like `&lparen;{ x = 5 }}` doesn't return anything.
  - Expressions like `&lparen;{ 5 + 5 }}` will return result. In this example it's `"10"`.
