...
description: Learn how to upgrade your Textwire code from v2 to v4
outline: deep
---
...
- Change import paths from `v2` to `v4`
...
Follow the steps below to upgrade your Textwire code to v4.

### 1. New Import Path

Change all the imports in your code from `github.com/textwire/textwire/v2` to `github.com/textwire/textwire/v4`
...
Applications using Go versions below 1.25 will not work with Textwire v4.
:::
...
:::warning Critical
Using `global` as a variable name will cause compilation errors in v4. This variable name is now reserved for internal use.
:::
...
In Textwire v2, custom functions would return the receiver type. For example, if you define a custom function for strings, it would return string. In Textwire v4, custom functions for all types return type `any`. Check if you have custom functions defined in your Go code.
...
Textwire v4 puts higher precedence on function call and in code like this <code v-pre>{{ !"aaa".contains("a") }}</code> it would first evaluate `"aaa".contains("a")` and then apply `!` operator. Check if you use the functions with prefix expressions like `!` and `-` and make sure they returns the result you expect.
...
Textwire v2 is using `.tw.html` file extension by default, in v4 this file default file extension was changed to `.tw`. If you use `.tw.html` for your Textwire files, you can resolve this in 2 ways:
...
Rename Textwire file extensions from `.tw.html` to `.tw` and make sure you don't have `TemplateExt` configuration setup. In Textwire v4 `TemplateExt` is set to `.tw` by default.

:::info Custom Extensions
You can set any extension for Textwire that you want, refer to [configurations](/v4/api/configurations) page for more details. But we recommend using `.tw`.
:::

### 8. Components Scope Fix

Components in **Textwire v2** would pass variables to their children automatially without manual passing. It was a bug. In **Textwire v4** each component has its scope. You need to pass data manually if you were using this:
...
Fixed variable leak from template to layout non-explicitly. In Textwire v2, if you had a variable in your template, it would be accessible in your layout without passing it explicitly. In Textwire v4, this is not available anymore.

If you were utilizing this behavior, you can fix it by using [Global Data](/v4/api/configurations#global-data). Use it to pass variables into all of your Textwire files.
