...
For detailed instructions on using `textwire.NewTemplate`, refer to the [Usage with Templates](/v4/api/template-usage) guide. Below is a basic example of setting configurations with `textwire.Configure`:
...
| `TemplateFS`      | Provides an optional fs.FS filesystem for template access. Use this field to embed templates into your binary using Go's embed package. [Read more](/v4/api/template-embedding).<br><br>- Type: `fs.FS`<br>- Default: `os.DirFS(TemplateDir)` |
...
| `DebugMode`       | A flag that enables debug mode. When enabled, error messages are displayed in the browser. [Read more](/v4/api/error-handling).<br><br>- Type: `bool`<br>- Default: `false`                              |
...
The `global` identifier is a reserved word and cannot be used for variable names. Read more about [reserved variables](/v4/#reserved-variable-names).
