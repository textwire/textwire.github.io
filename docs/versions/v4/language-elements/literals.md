...
When you print a string, it will be automatically escaped. If you want to print a string without escaping it, you can use the [raw()](/v4/functions/str#raw) function on strings. Example:
...
- **Indexing strings.** You cannot use `[number]` to access string character, instead, use [at()](/v4/functions/str#at) function.
...
- **Save accessing.** You can use [get()](/v4/functions/obj#get) function to safely access object properties without worrying about errors. Example:
    ```textwire
    {{ user = {age: 25, name: {first: 'Anna', last: 'Cho'}} }}
    {{ user.get('name.first') }}
    ```
